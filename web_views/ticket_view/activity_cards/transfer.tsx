        ...
        >
            {
                ({from, to}: any): React.ReactNode => {
                    ...
                    const content = [
                        <List.Item.Meta
                            key={0}
                            title={<Typography.Text style={{fontSize: 14, color: theme.white, fontWeight: 100}}>{t('ticket_activity_card_new_owner')}</Typography.Text>}
                            description={<Address address={to && to.length ? to[0] : undefined} size={16} color={theme.white}/>}
                        />,
                        <List.Item.Meta
                            key={1}
                            title={<Typography.Text style={{fontSize: 14, color: theme.white, fontWeight: 100}}>{t('ticket_activity_card_previous_owner')}</Typography.Text>}
